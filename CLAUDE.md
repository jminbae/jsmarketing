...
### 2026-07-05
- **외부 앱 연동(md-share) 추가**: 별도 Next.js 앱(레포 `jminbae/md-share`, 자체 Vercel 프로젝트, 공개 URL `https://mymdshare.vercel.app`)을 스튜디오 홈에 **외부링크 카드**로 붙임. ⚠️ 주의: Vercel에 옛/타인 배포본 `mdshare.vercel.app`(타인 소유)·`md-share-kappa.vercel.app`(구 주소)이 따로 있으니 헷갈리지 말 것 — 현재 정식 주소는 `mymdshare.vercel.app`(프로젝트 "md-share"의 프로덕션 별칭). 서버·업로드가 필요한 앱이라 스튜디오 안 브라우저 도구로는 합칠 수 없어 링크로 연결(각자 배포 유지).
